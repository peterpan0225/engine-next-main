...
export default function TagSection({ tagPageDetails }) {
  ...
  const handleLoadMore = async () => {
    try {
      ...
      const { tagPageDetails } = pageProps;
      setHasMore(tagPageDetails?.haveNextPage ?? false);
      setNextPostUri(tagPageDetails?.nextPageUrl);
      setInitialPages([
        ...initialPages,
        {
          pageDetails: tagPageDetails,
          firstPage: false,
          id: generateUniqueId(),
        },
      ]);
